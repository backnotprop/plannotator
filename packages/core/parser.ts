/**
 * @plannotator/core - Markdown Parser
...
/**
 * Export annotations as a formatted diff/feedback document.
 * Used to generate human-readable feedback for Claude.
 */
export const exportDiff = (blocks: Block[], annotations: Annotation[], globalAttachments: string[] = []): string => {
  ...
  sortedAnns.forEach((ann, index) => {
    ...
    // Include tag prefix if present (e.g., "## 1. @FIX Remove this")
    // Include [MACRO] flag if set (indicates cross-document impact)
    const tagPrefix = ann.tag ? `${ann.tag} ` : '';
    const macroFlag = ann.isMacro ? '[MACRO] ' : '';
    output += `## ${index + 1}. ${tagPrefix}${macroFlag}`;
