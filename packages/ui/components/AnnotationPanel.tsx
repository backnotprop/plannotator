...
export const AnnotationPanel: React.FC<PanelProps> = ({
  ...
  const sortedCodeAnnotations = [...codeAnnotations].sort(
      ...
      <div className="border-b border-border/50">
        <div className="flex h-10 items-center justify-between px-3">
          <div className="flex items-center gap-2">
            <h2 className="text-[10px] font-semibold uppercase tracking-widest text-foreground/70">
              Annotations
            </h2>
            {totalCount > 0 && (
        ...
        {otherFileAnnotations && otherFileAnnotations.count > 0 && (
          ...
          >
            +{otherFileAnnotations.count} in {otherFileAnnotations.files} other
            file{otherFileAnnotations.files === 1 ? "" : "s"}
          </button>
        )}
      </div>
              ...
              {timelineEntries.map((entry) =>
                entry.kind === "plan" ? (
                    ...
                    onEdit={
                      onEdit
                        ? (updates: Partial<Annotation>) =>
                            onEdit(entry.annotation.id, updates)
                        : undefined
                    }
                  />
                ) : (
                  <CodeAnnotationCard
                    ...
                    isMe={isCurrentUser(entry.annotation.author)}
                    onSelect={() =>
                      onSelectCodeAnnotation?.(entry.annotation.id)
                    }
                    onDelete={() =>
                      onDeleteCodeAnnotation?.(entry.annotation.id)
                    }
                    onEdit={
                      onEditCodeAnnotation
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isSelected
          ? "bg-surface-1 ring-1 ring-border/50"
