...
## Section references in comment text (unreleased)

One additive seam plus one new method on an existing handle. **No version assigned** — the maintainer picks it at release; nothing under `packages/core` changed, so this is a ui-only publish.

### `AnnotationPanel` `onNavigateAnchor?: (hash: string) => void`

An annotation that names another section in prose ("this contradicts 3.1") makes the reader hold the reply in their head while they go find 3.1. The panel now renders `#` followed by a heading's exact text as a link to that heading.

1. **Opt-in and inert by default.** The text is parsed only when BOTH `onNavigateAnchor` and a non-empty heading index are present; otherwise `parseSectionRefs` returns the original string and the same single `<p>` renders. A host that passes nothing gets byte-identical output.
2. **The document defines the vocabulary.** A reference resolves by exact match against this document's heading text — no numbering grammar to get wrong, and no configuration. A title used by two headings resolves to neither.
3. **Non-references stay plain by construction**, because resolution requires a heading to exist: `#fff` and `#123` need a heading literally named `fff` / `123`. A `#` welded to a word (`C#`) and a `#` followed by whitespace (markdown heading intent) are both skipped before lookup, and backtick spans are never scanned.
4. **Longest match wins**, so a document with both `3.1` and `3.1 Scope` resolves `#3.1 Scope` to the longer heading.
5. **Clicks do not select the card.** The link stops propagation, so navigating to a referenced section never also scrolls to the annotation's own highlight.

Only `AnnotationCard` renders these. `CodeAnnotationCard` is untouched: code-review annotations address files and lines, and a review surface has no heading blocks.

### `ViewerHandle.scrollToAnchor: (hash: string) => boolean`

`Viewer` already had this callback for in-document anchor links; it is now on the imperative handle so the panel can drive it. Returns `false` when the anchor is not in the current document.

It is **required, not optional**, deliberately: the interface is implemented inside this package only (`Viewer`, `HtmlViewer`) and consumed everywhere else through a ref, so requiring it costs no host anything while forcing both implementations to stay in step — `HtmlViewer` was caught by `tsc` and implements it as `() => false`, since a raw HTML page has no markdown heading blocks to resolve against. A host that holds a `ViewerHandle` ref is unaffected; only a host writing its own `forwardRef<ViewerHandle, …>` component would need to add it, which is not a supported shape.

### `utils/sectionRefs` (pure, exported)

`buildSectionRefIndex(blocks)` → `Map<headingText, anchorId>` over `buildHeadingSlugMap`, minus ambiguous titles. `parseSectionRefs(text, index)` → `Array<string | { label, anchor }>`. Both are DOM-free and dependency-free; `packages/ui/utils/sectionRefs.test.ts` pins the particle-boundary, longest-match, duplicate-title, `C#`, `# `, backtick, and no-heading cases.
