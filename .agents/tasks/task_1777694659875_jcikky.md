...
trackerStatus:
  type: task
  title: "E2E spec 05: review mode (diff types, async feedback, git-add)"
  status: to-do
  priority: high
  tags: ["plannotator", "e2e", "testing", "playwright", "review"]
  created: 2026-05-02T04:04:19.876Z
  updated: 2026-05-04T08:00:36.000Z
  progress: 0
...
Parent gate: G3 (Cross-slice E2E specs). Alias: E05.
Depends on: NIM-4 (router), NIM-6 (submit/wait/clear), NIM-23 (infrastructure).
Semantic deps: None.

Setup: tmp git repo, commit baseline, modify a file to create a known diff (use `make-repo.sh`).
