...
Process rule:
- This proof task must be authored separately from implementation, and [[S-5]] writers may not weaken or rewrite the tests after handoff.
...
Accepted child proof commit `48ec43a` and integrated it into `main`. Verifying in `main` with `bun test tests/nim-17.submit-wait-proof.test.ts`; expected red phase is the missing daemon-owned `/api/submit`/`/api/wait`/`/api/clear` surface for [[S-5]].
