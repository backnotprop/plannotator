...
Verification rules:
- This task consumes the umbrella proof policy from [[S-9.5]] and the paired proof tasks for each implementation slice rather than inventing late test coverage after implementation.
...
- Run the real end-to-end proof harness from [[S-9.5]].
- Run the paired TDD suites for [[S-1]] through [[S-9]].
...
Verification rules:
- This task consumes the proof harness defined in [[S-9.5]] rather than inventing late test coverage after implementation.
- No mocks, no faked data, no substitute backends, and no tests that only prove internal consistency.
- Validation must run real fixtures, real commands, real servers, and real user-visible workflows.
