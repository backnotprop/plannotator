...
Each task owns one executable E2E proof area. Status lives in the task card frontmatter; do not copy task status into this phase body.
...
| [[TASK-E07]] | Wait behavior, buffered verdict recovery, multi-waiter behavior, JSON wait output | `tests/e2e/specs/07-wait-broadcast.spec.ts` |
| [[TASK-E08]] | Crash recovery for submitter, waiter, daemon, and verdict persistence boundaries | `tests/e2e/specs/08-crash-recovery.spec.ts` |
...
| [[TASK-E15]] | Packaging, install path, deleted surfaces, stale docs checks, final surface consistency | `tests/e2e/specs/15-packaging.spec.ts` |

Concurrency coverage is distributed by surface: multi-waiter behavior belongs to [[TASK-E07]], concurrent CLI submit belongs to [[TASK-E04]] and [[TASK-E03]], concurrent Claude hook identity belongs to [[TASK-E13]], and stale verdict isolation belongs to [[TASK-E03]].
