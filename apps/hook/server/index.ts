...
} from "@plannotator/server/review-workspace";
...
function parseOpenCodeBridgeInput<T extends object>(
  ...
  try {
    ...
    console.error(
      `Failed to parse ${mode} input: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(1);
  }
}
...
function getBridgeSharingEnabled(input: OpenCodeBridgeInput): boolean {
  return typeof input.sharingEnabled === "boolean"
    ? input.sharingEnabled
    : sharingEnabled;
}

function getBridgeShareBaseUrl(input: OpenCodeBridgeInput): string | undefined {
  return typeof input.shareBaseUrl === "string" && input.shareBaseUrl
    ? input.shareBaseUrl
    : shareBaseUrl;
}

function getBridgePasteApiUrl(input: OpenCodeBridgeInput): string | undefined {
  return typeof input.pasteApiUrl === "string" && input.pasteApiUrl
    ? input.pasteApiUrl
    : pasteApiUrl;
}

function normalizeOpenCodeBridgeAgents(
  value: unknown,
): OpenCodeBridgeAgent[] | undefined {
...
if (args[0] === "sessions") {
  ...
  if (result.exit) {
    ...
    console.log(
      jsonFlag ? JSON.stringify(output) : JSON.stringify(output, null, 2),
    );
  }
  process.exit(0);
  ...
  let workspace:
    | Awaited<ReturnType<typeof buildLocalWorkspaceReview>>
    | undefined;
  ...
  if (isPRMode) {
    ...
    const managedVcs = await detectManagedVcs(
      process.cwd(),
      reviewArgs.vcsType,
    );
    ...
    if (managedVcs || forcedVcs) {
      ...
      if (workspace.repos.length === 0) {
        console.error(
          "Not in a VCS repo and no nested Git/JJ repositories were found.",
        );
        process.exit(1);
      }
  ...
  if (stdinFlag) {
    ...
    if (rolloutPath) {
      ...
      recentMessages = getRecentCodexMessages(
        rolloutPath,
    ...
    if (process.env.PLANNOTATOR_DEBUG) {
      console.error(
        `[DEBUG] Droid CWD session logs (mtime): ${cwdLogs.length ? cwdLogs.join(", ") : "(none)"}`,
      );
      if (cwdLogs.length === 0) {
        console.error(
          `[DEBUG] Droid ancestor walk: ${ancestorLogs.length ? ancestorLogs.join(", ") : "(none)"}`,
        );
      }
    }
    ...
    if (droidLog) {
      recentMessages = getRecentRenderedMessages(
        droidLog,
        RECENT_MESSAGES_LIMIT,
      );
      lastMessage = recentMessages[0] ?? null;
    ...
    function tryLogCandidates(label: string, getPaths: () => string[]): void {
      ...
      for (const logPath of paths) {
        const recent = getRecentRenderedMessages(
          logPath,
          RECENT_MESSAGES_LIMIT,
        );
        if (recent.length > 0) {
  ...
  const result =
    timeoutSeconds === null
      ...
      : await new Promise<Awaited<ReturnType<typeof server.waitForDecision>>>(
          (resolve) => {
  ...
  const reviewArgs = parseReviewArgs(
  ...
  if (isPRMode) {
    ...
    try {
      ...
      const cliName = getCliName(prRef);
      console.error(
        err instanceof Error ? err.message : `${cliName} auth check failed`,
      );
      process.exit(1);
    }
    ...
    try {
      ...
      console.error(
        err instanceof Error
          ? err.message
          : `Failed to fetch ${getMRLabel(prRef)} ${getMRNumberLabel(prRef)}`,
      );
      process.exit(1);
    }
  } else {
  ...
  const recentMessages = Array.isArray(input.recentMessages)
    ? input.recentMessages
        .map(
          (
            message,
          ): { messageId: string; text: string; timestamp?: string } | null => {
  ...
  if (!lastMessage) {
    console.error(
      "No assistant message found in opencode-annotate-last input.",
    );
    process.exit(1);
  }
