...
| `PLANNOTATOR_PORT` | random (local) / `19432` (remote) | Fixed server port or inclusive range. A range selects the first available port. Useful for forwarding or concurrent remote sessions. |
| `PLANNOTATOR_PUBLIC_URL` | unset | Full HTTP(S) origin advertised in remote mode behind a reverse proxy. The local runtime port is not appended. For a port range, this origin applies only to the first port; later ports use `PLANNOTATOR_URL_HOST` or `localhost`. Binding is unaffected and local sessions ignore the override. Can also be set via `~/.plannotator/config.json` (`{ "publicUrl": "https://plannotator.example.com" }`). |
| `PLANNOTATOR_URL_HOST` | unset (`localhost`) | Display-only hostname for advertised session URLs in remote mode (e.g. a Tailscale MagicDNS name). The runtime-chosen port is always appended; binding is unaffected and local sessions ignore the override. Used when `PLANNOTATOR_PUBLIC_URL` is unset or a port range selects a later port. Can also be set via `~/.plannotator/config.json` (`{ "urlHost": "host" }`). |
