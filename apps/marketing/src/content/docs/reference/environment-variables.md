...
| `PLANNOTATOR_SKIP_VIBE_INSTALL` | off | Same opt-out for the Mistral Vibe integration (the managed `hooks.toml` block and skills under `$VIBE_HOME`, default `~/.vibe`). Config key `skipInstall.vibe`; flag `--skip-vibe`. Read by the install scripts only. |
