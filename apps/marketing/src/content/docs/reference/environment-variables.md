...
| `PLANNOTATOR_READY_FILE` | `~/.plannotator/ready.jsonl` | Side channel for the session URL. Plannotator appends one server-ready JSON line (`{url, isRemote, port}`) the moment the server binds, so `tail -n 1` is the newest session. Host plugins point this at their own temp file; leave it unset to use the default. |
