...
| `PLANNOTATOR_INSTALL_CALLDIFF` | off | Set to `1` / `true` / `yes` to also install the optional pinned CallDiff runtime used by code review's Call Flow layer (large: roughly 800 MB on disk, needs Node.js 22+). It is not installed by default; the normal path is the in-app install offered by the Call flow panel when the layer is enabled. Equivalent to `--with-call-flow` (PowerShell: `-WithCallFlow`) or `{ "installCallFlow": true }` in `config.json`; flag > env var > config. Read by the install scripts only. |
| `PLANNOTATOR_CALLDIFF_PATH` | unset | Development override pointing at a built CallDiff `0.4.1` package root with the pinned Tree-sitter grammar set installed. Normal installs use Plannotator's managed runtime. |
