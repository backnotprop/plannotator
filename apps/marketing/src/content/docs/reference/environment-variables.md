...
| `PLANNOTATOR_PRESENTER` | unset | Path to an executable implementing Plannotator's external presenter protocol. The server sends one JSON record on the child's stdin (`present` with the session URL and kind; `dismiss` with the handle on session stop) and reads one JSON record from stdout, presenting the review URL instead of opening the browser — used by terminal hosts such as Herdr. An explicitly-set env var wins over the config-file presenter; an explicit empty value disables it. Can also be set via `~/.plannotator/config.json` (`{ "presenter": { "command": "/path/to/presenter", "when": "herdr" } }`); config-file presenters default to `"when": "herdr"` (active only inside a Herdr pane), while `"when": "always"` enables them everywhere. `PLANNOTATOR_SKIP_BROWSER_OPEN=1` wins over the presenter: neither the presenter nor a browser is invoked. Without a presenter, browser opening is unchanged. |
