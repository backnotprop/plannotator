...
**Compressed app page for network sessions (#1617):** remote-mode and `--tailscale` sessions serve the single-file app HTML brotli-compressed (gzip for clients without `br`, e.g. WebKit over plain http), negotiated on `Accept-Encoding`, with `Vary: Accept-Encoding`; the brotli pass (q5, ~0.2 s for the plan page) starts at server start and is cached per process. Plain local sessions serve the page exactly as before, because on loopback compression only added latency. One shared helper, `packages/shared/app-html.ts` (vendored to Pi), serves every app-HTML route in both runtimes; API responses are never compressed. The single-file builds also drop KaTeX's `woff`/`ttf` fallbacks (`build/katex-woff2-only.ts`); every supported browser loads `woff2`.
