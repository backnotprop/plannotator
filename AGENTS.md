...
| `PLANNOTATOR_INSTALL_CALLDIFF` | **Read by the install scripts only.** Set to `1` / `true` / `yes` to ALSO install the optional pinned CallDiff runtime used by code review's Call Flow analysis (large: roughly 800 MB on disk, needs Node.js 22+). The runtime is strictly opt-in and is NOT installed by default; the normal path is the in-app install: enable Call flow in the review UI and click Install in the Call flow panel (`POST /api/call-flow/install`), which works without reinstalling Plannotator. Equivalent to the `--with-call-flow` flag (PowerShell: `-WithCallFlow`) or `{ "installCallFlow": true }` in `~/.plannotator/config.json`; precedence is flag > env var > config. `--minimal` always excludes it. Off by default. |
| `PLANNOTATOR_CALLDIFF_PATH` | Development override for a built CallDiff package root. The package must be version `0.4.1`, include `dist/index.js`, and have every pinned Tree-sitter grammar installed under its own `node_modules`. Normal installs use the managed runtime under the Plannotator data directory. |
...
| `/api/call-flow`      | GET    | Runs snapshot-bound CallDiff analysis for the active Git review (`?snapshot=<id>` required). Returns bounded call trees, canonical colorless raw output, and a per-file impact index; stale, disabled, unsupported, unavailable, and error states are explicit. |
| `/api/call-flow/install` | POST | Starts the opt-in CallDiff runtime install in the background (single-flighted: concurrent POSTs join the in-flight install). Runs a Node >= 22 preflight before any download and returns the resulting status record; a missing Node is a distinct immediate `{ state: "error", reason: "node-unavailable" }`. Cross-origin POSTs (Origin header not matching the request host) are rejected with 403 because this endpoint triggers a large download and build. |
| `/api/call-flow/install-status` | GET | Poll the in-app runtime install: `{ state: "idle" \| "running" \| "done" \| "error", stage?, error?, reason? }` with `stage` one of `downloading` / `verifying` / `installing-deps` / `building`. `done` persists (the next capability advert resolves available without a server restart, because completion invalidates the 30s runtime probe cache); `error` persists until the next install POST retries. |
| `/api/review-analysis` | POST  | Persists `{ semanticDiff, callFlow }` booleans and immediately returns fresh capability adverts for the current view. |
