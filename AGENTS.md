...
**Anchoring an annotation to the document.** An external annotation quotes the markdown SOURCE (that is what `/api/plan` serves) and is pinned by searching the RENDERED DOM, so the two diverge wherever the renderer consumes syntax, drops a delimiter, or draws text of its own. That divergence has no inverse worth writing: `packages/ui/utils/renderedText.tsx` runs the real renderer (`parseMarkdownToBlocks` → `BlockRenderer` → `renderToStaticMarkup` into an inert `<template>`) over the quote and compares its text, so a markdown feature is covered the day it renders rather than after someone notices the missing highlight. Both sides are read through `visibleText`, never `textContent`, which skips `[aria-hidden="true"]` and `[data-decorative]` — see the rule in `packages/ui/CLAUDE.md` before adding renderer chrome. `useAnnotationHighlighter`'s ladder is then only: the literal quote, the rendered quote, and a last-resort match that drops markdown noise and whitespace from both sides for a quote that cuts a block in half (`Install** now`). `stripInlineMarkdown` is no longer part of it and now serves `slugifyHeading` alone, which stays a pure DOM-free function.
