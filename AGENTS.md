...
**The built `index.html` is a packed bundle.** Every host still ships and embeds ONE file per app, but the browser downloads only the page and its entry chunk: `build/pack-app.ts` keeps Vite's code-split output (lazy chunks, fonts, images) behind a marker after the page, and every server answers `/assets/*` from that pack through `createAppHandler` (`packages/server/shared-handlers.ts`, mirrored in `apps/pi-extension/server/helpers.ts`) over `packages/shared/packed-app.ts`, gzipping text on first request. The first load moves ~2 MB instead of ~25 MB (plan) or ~18 MB (review), which is what makes a session usable over a tailnet or an SSH tunnel.
