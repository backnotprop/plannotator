...
Code-review drafts (`/api/draft`) are keyed by `contentHash(rawPatch)`, so a local review whose diff changes still starts without its old draft (unchanged, out of scope). **PR mode only** additionally stores the draft under a stable target key, `prDraftTargetKey(meta, scope)` = `pr-` + hash of platform + host + repo (`owner/repo` or GitLab `projectPath`, lower-cased) + PR number + diff scope (`layer` / `full-stack` are different patches). All of it lives in `packages/shared/review-draft.ts` (vendored to Pi; both review servers route `/api/draft`, `/api/feedback` and `/api/exit` through it; without a target key every call is the plain `draft.ts` call). Load tries the patch key first and returns it unless the target copy is strictly newer by `draftGeneration`; the target copy is stamped server-side with the `patchKey` it was saved on and, when served for a different patch, the response carries `patchChanged: true` (clients cannot forge either field). Delete clears the patch key, the target key, and the patch key the target copy names. The target key's tombstone guards the whole logical draft: a save at or below it is rejected under BOTH keys, and a patch-key copy at or below it is not served, so a stale tab on a pre-push patch cannot resurrect a submitted draft.

Client side (`packages/review-editor`): in PR mode `withPRContext` records `anchorText` (the anchored diff lines' text, from the patch hunks) on each line comment. Restoring a `patchChanged` draft runs `markOutdatedCodeAnnotations` (`utils/codeAnnotationAnchor.ts`): a line comment whose file is still in the diff and whose `anchorText` still matches at the same side/lines is untouched; every other line comment — including ones with no `anchorText` — gets `outdated: true` and keeps its old line numbers (never dropped, never moved). File and general comments, and comments bound to another PR, pass through. Outdated comments are not drawn on the diff (the review-state context filters them), show an "Outdated" chip and an Edit action in the sidebar, export under `OUTDATED_ANNOTATION_LABEL`, and are posted to GitHub/GitLab in the review body rather than inline.
