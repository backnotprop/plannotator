...
- **In-place switches**: `/api/pr-switch` and `/api/pr-diff-scope` responses carry `draftState: { found, draftGeneration }` for the new keys. The client's `adoptDraftTarget` raises its generation to that floor (otherwise every save after switching onto a previously submitted target would be refused) and, when the target holds items the session does not have, offers them through the restore banner as a MERGE (never replacing the session's own comments, and never re-offering a comment the reviewer deleted this session). While that offer is pending, autosave neither saves nor deletes, so the switch itself (which replaces viewed files) cannot overwrite or tombstone the new target's draft before the reviewer answers.
