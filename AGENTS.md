...
| `PLANNOTATOR_ANNOTATE_MESSAGE_ANCHORING` | Set to `0` / `false` to stop the Pi extension prepending the annotated-response excerpt when delivering `/plannotator-last` message feedback (#1334). The anchor re-sends up to 1,000 characters of the target assistant response — useful when the session has moved on or feedback targets an older message, redundant when the model still holds the response in context. Default: enabled (anchor only when the target may be stale: different session, or entries added after it). Can also be set via `~/.plannotator/config.json` (`{ "annotateMessageAnchoring": false }`); the env var takes precedence. |
